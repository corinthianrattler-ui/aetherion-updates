/* Aetherion Reforged v1.60.0 — fitted modular Valkorion and strict equipment visibility. */
'use strict';
(()=>{
 const VERSION='1.60.0';
 const MODEL_PATH='assets/v106/valkorion_modular.glb';
 const FOUNDATION=Object.freeze([
  'foundation__head','foundation__torso','foundation__arms','foundation__legs','foundation__feet'
 ]);
 ...
 function repairArt(){
  ...
  }catch(error){console.warn('[Aetherion 1.60.0 art]',error)}
 }
 function createCanvas(host){
  host.replaceChildren();
  const canvas=document.createElement('canvas');
  canvas.dataset.v98Viewer='true';canvas.dataset.v106Valkorion='true';
 ...
 function applyCreator(viewer){
  if(!viewer?._v106Valkorion||!viewer.model)return viewer;
  const creator=S?.v98?.creator||{height:1,frame:1},model=viewer.model;
  model.userData.v106BaseScale??={x:model.scale.x,y:model.scale.y,z:model.scale.z};
  const base=model.userData.v106BaseScale,frame=Math.max(.90,Math.min(1.10,+creator.frame||1)),height=Math.max(.92,Math.min(1.08,+creator.height||1));
 ...
 function attachIdle(viewer){
  if(!viewer?._v106Valkorion||viewer._v106Idle||!viewer.model)return viewer;
  viewer._v106Idle=true;
  ...
  let last=0;
  function tick(now){
   if(viewer.disposed||!viewer._v106Idle)return;
 ...
 function applyVisibility(viewer,gear=S?.player?.equipment||{}){
  ...
  try{viewer.setNodesVisible?.(map)}catch(error){for(const[name,shown]of Object.entries(map))try{viewer.setNodeVisible?.(name,shown)}catch(_){ }console.warn('[Aetherion 1.60.0 visibility]',error)}
 ...
 async function createViewer(host){
  ...
  const viewer=await api.createViewer({canvas,container:host,glb:url,proceduralFallback:false,modelHeight:4.15,maxPixelRatio:1.75,onWebGLFailure:error=>console.error('[Aetherion 1.60.0 WebGL]',error),onError:error=>console.error('[Aetherion 1.60.0 model]',error)});
  viewer._v106Valkorion=true;
 ...
 async function sync(){
  ...
  try{
   ...
   if(viewer&&!viewer._v106Valkorion){try{viewer.dispose?.()}catch(_){}viewer=null;V98_PLAYER.viewer=null}
   if(!viewer){
    ...
    V98_PLAYER.viewer=viewer;V98_PLAYER.kind='valkorion_v106';
   }
   ...
   console.error('[Aetherion 1.60.0 player]',error);return null;
  }
 }
 function state(){
 ...
 }catch(error){console.warn('[Aetherion 1.60.0 hooks]',error)}
 try{V98_PLAYER.viewer?.dispose?.()}catch(_){}
 try{V98_PLAYER.viewer=null;V98_PLAYER.kind=null;V98_PLAYER.pending=false}catch(_){}
 try{
  let style=document.getElementById('aetherion-v106-modular-style');
  if(!style){style=document.createElement('style');style.id='aetherion-v106-modular-style';style.textContent='.v80-stage canvas[data-v106-valkorion]{filter:drop-shadow(0 12px 20px rgba(0,0,0,.38))}.v98-view-note::after{content:" · Valkorion gear is now fitted per equipped slot.";color:#c6a876}';document.head.appendChild(style)}
 }catch(_){}
 setTimeout(()=>{try{repairArt();if(typeof render==='function')render();if(typeof v98Schedule==='function')v98Schedule()}catch(error){console.warn('[Aetherion 1.60.0 start]',error)}},80);
 window.AetherionValkorionV106=Object.freeze({version:VERSION,model:MODEL_PATH,nodes:Object.freeze([...FOUNDATION,...GEAR_NODES]),visibility,sync,state,repairArt});
})();
