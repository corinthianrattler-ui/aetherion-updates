/* Aetherion Reforged v1.62.0 — fitted segmented Valkorion Dominus armor. */
'use strict';
(()=>{
 const VERSION='1.62.0';
 const MODEL_PATH='assets/v108/valkorion_modular.glb';
 const FOUNDATION=Object.freeze([
  'foundation__head','foundation__torso','foundation__arms','foundation__hands','foundation__legs','foundation__feet'
 ]);
 const DOMINUS_NODES=Object.freeze([
  'SLOT_HELMET','SLOT_GORGET','SLOT_PAULDRONS','SLOT_CUIRASS','SLOT_UNDERCOAT','SLOT_GAUNTLETS',
  'SLOT_BELT','SLOT_TROUSERS','SLOT_GREAVES','SLOT_BOOTS','SLOT_CLOAK','SLOT_SCABBARD'
 ]);
 const GEAR_NODES=Object.freeze([
  ...DOMINUS_NODES,'gear__royal_head','gear__royal_underlayer','gear__royal_body',
 ...
 function visibility(gear={}){
  ...
  for(const[slot,[id,nodes]]of Object.entries(ARMOR)){
   const shown=value(gear,slot)===id||isHouseArmor(slot,value(gear,slot));
  ...
  map['SLOT_SCABBARD']=map['gear__dominus_sword'];
  ...
  // The segmented Tripo master was authored as one complete armored figure,
  // without a matching naked body beneath it. Keep the foundation as an
  // underbody for partial outfits; hiding isolated regions creates literal
  // holes between fitted surface fragments. Once the complete armor is worn,
  // hide the old body so the supplied fitted master is shown untouched.
  const fullDominus=Object.entries(ARMOR).every(([slot,[id]])=>value(gear,slot)===id||isHouseArmor(slot,value(gear,slot)));
  map['foundation__head']=!map['SLOT_HELMET'];
  map['foundation__torso']=!(fullDominus||map['gear__royal_body']);
  map['foundation__arms']=!(fullDominus||map['gear__royal_shoulders']);
  map['foundation__hands']=!(fullDominus||map['gear__royal_hands']);
  map['foundation__legs']=!(fullDominus||map['gear__royal_legs']);
  map['foundation__feet']=!(fullDominus||map['SLOT_BOOTS']||map['gear__royal_feet']);
  return map;
 }
 function playerMode(){
 ...
 function repairArt(){
  ...
  }catch(error){console.warn('[Aetherion 1.62.0 art]',error)}
 }
 function createCanvas(host){
  host.replaceChildren();
  const canvas=document.createElement('canvas');
  canvas.dataset.v98Viewer='true';canvas.dataset.v108Valkorion='true';
 ...
 function applyCreator(viewer){
  if(!viewer?._v108Valkorion||!viewer.model)return viewer;
  const creator=S?.v98?.creator||{height:1,frame:1},model=viewer.model;
  model.userData.v108BaseScale??={x:model.scale.x,y:model.scale.y,z:model.scale.z};
  const base=model.userData.v108BaseScale,frame=Math.max(.90,Math.min(1.10,+creator.frame||1)),height=Math.max(.92,Math.min(1.08,+creator.height||1));
 ...
 function attachIdle(viewer){
  if(!viewer?._v108Valkorion||viewer._v108Idle||!viewer.model)return viewer;
  viewer._v108Idle=true;
  ...
  let last=0;
  function tick(now){
   if(viewer.disposed||!viewer._v108Idle)return;
 ...
 function applyVisibility(viewer,gear=S?.player?.equipment||{}){
  ...
  try{viewer.setNodesVisible?.(map)}catch(error){for(const[name,shown]of Object.entries(map))try{viewer.setNodeVisible?.(name,shown)}catch(_){ }console.warn('[Aetherion 1.62.0 visibility]',error)}
 ...
 async function createViewer(host){
  ...
  const viewer=await api.createViewer({canvas,container:host,glb:url,proceduralFallback:false,modelHeight:4.15,maxPixelRatio:1.75,onWebGLFailure:error=>console.error('[Aetherion 1.62.0 WebGL]',error),onError:error=>console.error('[Aetherion 1.62.0 model]',error)});
  viewer._v108Valkorion=true;
 ...
 async function sync(){
  ...
  try{
   ...
   if(viewer&&!viewer._v108Valkorion){try{viewer.dispose?.()}catch(_){}viewer=null;V98_PLAYER.viewer=null}
   if(!viewer){
    ...
    V98_PLAYER.viewer=viewer;V98_PLAYER.kind='valkorion_v108';
   }
   ...
   console.error('[Aetherion 1.62.0 player]',error);return null;
  }
 }
 function state(){
 ...
 }catch(error){console.warn('[Aetherion 1.62.0 preview hooks]',error)}
 try{V98_PLAYER.viewer?.dispose?.()}catch(_){}
 try{V98_PLAYER.viewer=null;V98_PLAYER.kind=null;V98_PLAYER.pending=false}catch(_){}
 try{
  let style=document.getElementById('aetherion-v108-modular-style');
  if(!style){style=document.createElement('style');style.id='aetherion-v108-modular-style';style.textContent='.v80-stage canvas[data-v108-valkorion]{filter:drop-shadow(0 12px 20px rgba(0,0,0,.38))}.v98-view-note::after{content:" · Segmented fitted-master armor.";color:#c6a876}';document.head.appendChild(style)}
 }catch(_){}
 setTimeout(()=>{try{repairArt();if(typeof render==='function')render();if(typeof v98Schedule==='function')v98Schedule()}catch(error){console.warn('[Aetherion 1.62.0 start]',error)}},80);
 window.AetherionValkorionV108=Object.freeze({version:VERSION,model:MODEL_PATH,nodes:Object.freeze([...FOUNDATION,...GEAR_NODES]),visibility,sync,state,repairArt});
})();
