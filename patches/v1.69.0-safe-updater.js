/* Aetherion Reforged v1.69.0 — staged updater, automatic rollback, and Safe Start. */
'use strict';
(()=>{
 const VERSION='1.69.0';
 const BUNDLED_VERSION='1.69.0';
 const ANDROID_BUILD=185;
 const CHANNEL_URL='https://raw.githubusercontent.com/corinthianrattler-ui/aetherion-updates/main/channel.js';
 ...
 function sanitizeRelease(input,{sources=true}={}){
  ...
  return{version:String(input.version),build:Number(input.build)||0,minimumBundled:String(input.minimumBundled||BUNDLED_VERSION),modules:cleanedModules,assets,notes,releasedAt:String(input.releasedAt||'')};
 }
 ...
 function loadChannel(){
  ...
  const script=document.createElement('script'),timeout=setTimeout(()=>runtime.checking?.reject(Error('The update channel did not answer.')),15000);
  function cleanup(){clearTimeout(timeout);script.remove?.();runtime.checking=null}
  script.async=true;script.src=CHANNEL_URL+'?check='+Date.now();
  script.onerror=()=>runtime.checking?.reject(Error('The update channel could not be reached.'));
  script.onload=()=>setTimeout(()=>{if(!finished)runtime.checking?.reject(Error('The update channel response was incomplete.'))},50);
 ...
 function refreshButtons(){
  ...
  document.querySelectorAll?.('[data-aetherion-updates]').forEach(control=>{
   control.querySelector?.('.aetherion-update-badge')?.remove?.();
   if(available){const badge=document.createElement('span');badge.className='aetherion-update-badge';badge.setAttribute('aria-label','Update available');control.appendChild(badge)}
  });
 }
 ...
 function mountInterface(){
  ...
  if(typeof MutationObserver==='function'&&!runtime.observer){runtime.observer=new MutationObserver(mountButtons);runtime.observer.observe(document.documentElement,{childList:true,subtree:true})}
 }
 ...
 function open(){
  ...
  const version=document.createElement('div');version.className='au-version';version.textContent=`Built-in ${BUNDLED_VERSION} · Running ${runtime.safeSession?'Safe Start':currentVersion()} · Android build ${ANDROID_BUILD}`;
  ...
  const check=addAction(actions,'CHECK NOW',async()=>{
   ...
   try{
    const release=await checkForUpdates(),newer=compareVersions(release.version,currentVersion())>0;
    status.textContent=newer?`Update ${release.version} is verified and ready to stage.`:`You are current. Version ${currentVersion()} is the latest stable release.`;
    for(const line of release.notes){const item=document.createElement('li');item.textContent=line;notes.appendChild(item)}
    if(newer)addAction(actions,`STAGE ${release.version}`,()=>{
 ...
 Object.assign(existing,{
  ...
  state(){const stored=readState();return{version:VERSION,bundled:BUNDLED_VERSION,current:currentVersion(),safeSession:runtime.safeSession,active:stored.active?.version||null,previous:stored.previous?.version||null,staged:stored.staged?.version||null,lastError:runtime.lastError}},
  _test:Object.freeze({compareVersions,sha256,fallbackSha256,sanitizeRelease,verifyRelease,readState,writeState,recoverFailedBoot,promoteStaged,currentVersion})
 });
