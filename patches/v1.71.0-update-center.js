/* Aetherion Reforged v1.71.0 — permanent, visible update center with real transfer progress. */
'use strict';
(()=>{
 const VERSION='1.71.0';
 const ANDROID_BUILD=188;
 ...
 const runtime={modal:null,observer:null,busy:false,candidate:null,lastBytes:0,lastTotal:0};

 if(window.AetherionUpdateCenterV171?.version===VERSION)return;
 ...
 function installRelease(ui,release){
  try{
   updater().stage(release);runtime.candidate=release;
   paintProgress(ui,runtime.lastBytes||1,runtime.lastTotal||runtime.lastBytes||1,'Installed and ready');
   ui.bar.style.width='100%';ui.bar.classList.remove('au-indeterminate');
   ui.status.textContent=`Update ${release.version} is installed and ready. Restart the game to activate it.`;
 ...
 function copyAddress(control,status){
  const done=()=>{control.textContent='COPIED';setTimeout(()=>{control.textContent='COPY ADDRESS'},1300)};
  if(navigator.clipboard?.writeText){navigator.clipboard.writeText(CHANNEL_DISPLAY_URL).then(done).catch(()=>fallbackCopy())}
  else fallbackCopy();
  function fallbackCopy(){
   const field=document.createElement('textarea');field.value=CHANNEL_DISPLAY_URL;field.style.position='fixed';field.style.opacity='0';
 ...
 function open(){
  ...
  const versions=text('div','auc-version',`Built-in ${VERSION} · Running ${installed.current||VERSION} · Android build ${ANDROID_BUILD}`);
  const route=text('div','auc-route');
  route.innerHTML='<b>Where this lives</b><span>Gold button above Systems — always visible</span><span>Opening Menu → GAME UPDATES</span><span>Systems → GAME UPDATES</span>';
  ...
  const status=text('div','auc-status',installed.staged
   ?`Update ${installed.staged} has downloaded. Restart the game to activate it.`
   :'Ready. Tap CHECK & DOWNLOAD to contact the update address.');
  ...
  const check=action(actions,'CHECK & DOWNLOAD',async()=>{
   ...
   try{
    const result=await downloadRelease((loaded,total)=>paintProgress(ui,loaded,total,'Downloading update index'));
    ...
    if(updater()._test.compareVersions(release.version,current)>0){
     status.textContent=`Download complete: ${formatBytes(result.wireBytes)} received and update ${release.version} verified. Tap INSTALL DOWNLOADED UPDATE.`;
     actions.replaceChildren();
     action(actions,'INSTALL DOWNLOADED UPDATE',()=>installRelease(ui,release),'au-primary');
     action(actions,'CLOSE',close);
    }else{
     status.textContent=`Download complete: ${formatBytes(result.wireBytes)} received and verified. You already have ${current}; no newer update is posted.`;
    }
   }catch(error){
  ...
  if(installed.staged){action(actions,'RESTART GAME',()=>location.reload(),'au-primary')}
  action(actions,'CLOSE',close);
  card.append(eyebrow,heading,versions,route,endpoint,status,progress,progressText,notes,actions);
 ...
 function launcher(){
  let control=document.getElementById('aetherion-update-launcher');
  if(!control){control=text('button','aetherion-update-button','⬇ GAME UPDATES');control.id='aetherion-update-launcher';control.type='button';control.dataset.aetherionUpdates='true';(document.body||document.documentElement).appendChild(control)}
  control.dataset.aetherionUpdateCenter='true';return control;
 }
 function mount(){
  launcher();
 ...
 function styles(){
  ...
  const css=text('style');css.id='aetherion-update-center-style';css.textContent=`
   #aetherion-update-launcher{position:fixed!important;z-index:2147483640!important;right:12px!important;bottom:calc(env(safe-area-inset-bottom,0px) + 82px)!important;width:auto!important;min-height:46px!important;padding:10px 15px!important;border:1px solid #b28a45!important;border-radius:12px!important;background:linear-gradient(180deg,#5c4024,#2b1a16)!important;color:#fff0bf!important;box-shadow:0 8px 24px #000c,0 0 0 1px #211!important;font:700 13px Georgia,serif!important;letter-spacing:.04em!important}
   ...
   .auc-eyebrow{font-size:11px;font-weight:700;letter-spacing:.18em;color:#aa8a56}.auc-card h2{margin:4px 0 3px;color:#e2c477;font-size:27px}.auc-version{margin-bottom:14px;color:#b9a9a4;font-size:13px}
   ...
   @media(max-width:430px){#aetherion-update-launcher{right:9px!important;bottom:calc(env(safe-area-inset-bottom,0px) + 76px)!important;min-height:42px!important;padding:8px 12px!important;font-size:12px!important}.auc-card{padding:16px}.auc-endpoint{grid-template-columns:1fr}.auc-copy{justify-self:start}}
  `;(document.head||document.documentElement).appendChild(css);
 }

 document.addEventListener('click',event=>{
  const target=event.target?.closest?.('[data-aetherion-updates],#aetherion-update-launcher');if(!target)return;
  event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();open();
 },true);
 function start(){styles();mount();if(!runtime.observer&&typeof MutationObserver==='function'){runtime.observer=new MutationObserver(mount);runtime.observer.observe(document.documentElement,{childList:true,subtree:true})}}
 window.AetherionUpdateCenterV171=Object.freeze({version:VERSION,androidBuild:ANDROID_BUILD,channel:CHANNEL_DISPLAY_URL,open,close,mount,state:()=>({...state(),busy:runtime.busy,lastBytes:runtime.lastBytes,lastTotal:runtime.lastTotal})});
