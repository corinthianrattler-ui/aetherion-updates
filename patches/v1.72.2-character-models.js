/* Aetherion Reforged v1.72.2 — Android-safe Valkorion and Lady Alexus models. */
'use strict';
(()=>{
 ...
 function setStage(node,state){
  ...
  stage.classList?.remove?.(
   'v80-3d-ready','v80-3d-loading','v80-2d-fallback','v170-model-error','v171-model-error','v172-model-error'
  );
 ...
 function applyBaseOrLord(viewer,selected){
  ...
  try{viewer.setNodesVisible?.(map)}catch(error){
   ...
   console.warn?.('[Aetherion 1.72.2 Valkorion visibility]',error);
  }
  return map;
 }
 ...
 function showValkorionError(node,error){
  runtime.valkorion.error=String(error?.message||error);if(!node)return;setStage(node,'v172-model-error');
  try{
  ...
  console.error?.('[Aetherion 1.72.2 Valkorion]',error);
 }
 ...
 async function mountAlexus(){
  ...
  try{
   ...
   console.error?.('[Aetherion 1.72.2 Alexus]',error);return null;
  }
 }
 function scheduleAlexus(){
  if(runtime.alexus.scheduled)return null;runtime.alexus.scheduled=true;
  const run=()=>{runtime.alexus.scheduled=false;mountAlexus().catch(error=>console.error?.('[Aetherion 1.72.2 Alexus mount]',error))};
 ...
 function wrapPersonViews(){
  ...
  }catch(error){console.warn?.('[Aetherion 1.72.2 Alexus profile hook]',error)}
  try{
  ...
  }catch(error){console.warn?.('[Aetherion 1.72.2 Alexus equipment hook]',error)}
  try{
  ...
  }catch(error){console.warn?.('[Aetherion 1.72.2 Alexus close hook]',error)}
 }
 function wrapGameRender(){
  if(runtime.renderWrapped)return true;
  try{
   if(typeof render!=='function')return false;
   if(render.__aetherionCharacterModelsV1722){runtime.renderWrapped=true;return true}
   const base=render;
   const wrapped=function(...args){
    const out=base.apply(this,args);
    if(typeof currentTab!=='undefined'&&currentTab==='equipment')scheduleValkorion();
    return out;
   };
   wrapped.__aetherionCharacterModelsV1722=true;render=wrapped;runtime.renderWrapped=true;return true;
  }catch(error){console.warn?.('[Aetherion 1.72.2 render hook]',error);return false}
 }
 function installStyles(){
  ...
  const css=document.createElement('style');css.id='aetherion-v172-character-models';css.textContent=`
   ...
   .v80-stage.v172-model-error .v80-valkorion-host{display:flex!important;opacity:1!important;visibility:visible!important;pointer-events:none!important}
   .v80-stage.v172-model-error .v80-fallback{opacity:0!important;visibility:hidden!important}
   .v80-stage.v172-model-error .v80-viewer-tools{display:none!important}
 ...
 }catch(error){console.warn?.('[Aetherion 1.72.2 viewer hooks]',error)}
 const start=()=>{try{tagAlexusModel();wrapGameRender();if(typeof currentTab!=='undefined'&&currentTab==='equipment')scheduleValkorion()}catch(error){console.warn?.('[Aetherion 1.72.2 start]',error)}};
