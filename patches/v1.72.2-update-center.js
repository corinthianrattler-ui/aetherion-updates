/* Aetherion Reforged v1.72.2 — menu-based update center with real transfer progress. */
'use strict';
(()=>{
 const VERSION='1.72.2';
 const ANDROID_BUILD=191;
 ...
 const APK_URL='https://github.com/corinthianrattler-ui/aetherion-updates/releases/download/v1.72.2/Aetherion_Reforged_v1.72.2_ANDROID_ASSET_FIX.apk';
 ...
 function open(){
  ...
  const nativeBuild=updater()?.androidBuild||ANDROID_BUILD;
  const versions=text('div','auc-version',`Built-in ${installed.bundled||VERSION} · Running ${installed.current||VERSION} · Android build ${nativeBuild}`);
  const route=text('div','auc-route');
  route.innerHTML='<b>Where this lives</b><span>Opening Menu → GAME UPDATES</span><span>Systems → GAME UPDATES</span><span>Full 3D build → DOWNLOAD FULL APK</span>';
  ...
  action(actions,'DOWNLOAD FULL APK',openFullApk);
  action(actions,'CLOSE',close);
 ...
 window.AetherionUpdateCenterV172=Object.freeze({version:VERSION,androidBuild:ANDROID_BUILD,channel:CHANNEL_DISPLAY_URL,apk:APK_URL,open,close,mount,state:()=>({...state(),busy:runtime.busy,lastBytes:runtime.lastBytes,lastTotal:runtime.lastTotal})});
