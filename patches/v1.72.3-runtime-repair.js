/* Aetherion Reforged v1.72.3 — startup, controls, and native-build identity repair. */
'use strict';
(()=>{
 ...
 function markBuild(){
  const ready=document.querySelector?.('.startReady');if(!ready)return;
  ready.textContent=`v${VERSION} REFORGED · Android build ${ANDROID_BUILD} · Native 3D models`;
 }
 ...
 function stopOpeningFilm(){
  try{
   if(typeof v12IntroTimer!=='undefined'){
    clearTimeout(v12IntroTimer);
    v12IntroTimer=null;
   }
   const video=document.getElementById?.('v12IntroVideo');
   if(video){
    try{video.pause?.()}catch(_){ }
    try{video.removeAttribute?.('src')}catch(_){ }
    try{video.load?.()}catch(_){ }
   }
   if(typeof v12IntroActive!=='undefined')v12IntroActive=false;
   if(typeof v12IntroReturnTab!=='undefined')v12IntroReturnTab='story';
  }catch(_){ }
 }
 ...
 function installStyles(){
  if(document.getElementById?.('aetherion-v173-runtime-style'))return;
  const style=document.createElement('style');style.id='aetherion-v173-runtime-style';style.textContent=`
 ...
 window.AetherionRuntimeRepairV172=Object.freeze({
  version:VERSION,androidBuild:ANDROID_BUILD,continue:continueTimeline,showOpeningMenu,mount:mountUpdatePaths,
  state:()=>({...runtime})
 });
