...
const patch=fs.readFileSync(path.join(__dirname,'..','patches','v1.60.0-valkorion-modular.js'),'utf8');
...
vm.runInContext(patch,context,{filename:'v1.60.0-valkorion-modular.js'});

const api=context.AetherionValkorionV106;
assert.equal(api.version,'1.60.0');
assert.equal(api.nodes.length,32);
assert.equal(new Set(api.nodes).size,32);
...
for(const slot of ['head','torso','arms','legs','feet'])assert.equal(api.visibility({})[`foundation__${slot}`],true);
...
assert.equal(lexical.AetherionValkorionV106.version,'1.60.0');
assert.equal(lexical.AetherionValkorionV106.visibility({body:'v98_royal_m_body'}).gear__dominus_cuirass,false);

console.log('v1.60.0 equipment visibility: all assertions passed');
