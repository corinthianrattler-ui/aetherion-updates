...
const patch=fs.readFileSync(path.join(__dirname,'..','patches','v1.62.0-valkorion-fitted.js'),'utf8');
...
vm.runInContext(patch,context,{filename:'v1.62.0-valkorion-fitted.js'});

const api=context.AetherionValkorionV108;
assert.equal(api.version,'1.62.0');
assert.equal(api.nodes.length,35);
assert.equal(new Set(api.nodes).size,35);

const shownGear=gear=>Object.entries(api.visibility(gear)).filter(([name,visible])=>(name.startsWith('gear__')||name.startsWith('SLOT_'))&&visible).map(([name])=>name).sort();
...
assert.equal(api.visibility({head:'v98_royal_m_head'}).SLOT_HELMET,false,'royal IDs must not also mount armor');
...
// Partial segmented armor keeps the intact foundation as its underbody.
...
assert.equal(visible.foundation__feet,false,'closed boots must hide bare foundation feet');

// The supplied armor is a single fitted master. A complete equipped set hides
// the old body so no peasant clothing can poke through its authored surfaces.
const fullDominus={
 head:'dominus_lord_helm',neck:'dominus_gorget',underlayer:'dominus_arming_doublet',body:'dominus_cuirass',
 shoulders:'dominus_pauldrons',hands:'dominus_gauntlets',waist:'dominus_belt',legs:'dominus_legplates',
 feet:'dominus_boots',cloak:'dominus_cloak'
};
visible=api.visibility(fullDominus);
for(const slot of ['head','torso','arms','hands','legs','feet'])assert.equal(visible[`foundation__${slot}`],false,`complete set must hide foundation ${slot}`);
...
assert.equal(lexical.AetherionValkorionV108.version,'1.62.0');
assert.equal(lexical.AetherionValkorionV108.visibility({body:'v98_royal_m_body'}).SLOT_CUIRASS,false);

console.log('v1.62.0 equipment visibility and body coverage: all assertions passed');
