...
const payload=manifest.payloads.find(item=>item.path==='patches/v1.69.1-start-menu-access.js');
assert(payload,'bundled startup patch is missing from the manifest');
assert(crypto.createHash('sha256').update(source).digest('hex')===payload.sha256,'manifest checksum differs from the startup patch');
let feed=null;
...
assert(feed?.release?.version==='1.69.2','stable channel version was not updated');
assert(feed.release.minimumBundled==='1.69.2','stable channel does not require the repaired APK');
assert(feed.release.modules.length===0,'bundled startup patch was unnecessarily offered for download');
