...
vm.runInContext(fs.readFileSync('patches/v1.69.2-safe-updater.js','utf8'),context,{filename:'v1.69.2-safe-updater.js'});
const api=window.AetherionUpdater,test=api._test;
assert(api.safeUpdaterVersion==='1.69.2','updater API missing');
assert(api.bundledVersion==='1.69.2','bundled version mismatch');
assert(api.androidBuild===186,'Android build mismatch');
assert(test.compareVersions('1.70.0','1.69.9')===1,'version ordering failed');
assert(test.compareVersions('1.69.2','1.69.2')===0,'version equality failed');
...
(async()=>{
 ...
 const release={version:'1.69.2',build:186,minimumBundled:'1.69.2',releasedAt:'2026-09-07T07:00:00Z',notes:['transport test'],modules:[],assets:{}};
 ...
 assert((await primaryCheck).version==='1.69.2','decoded channel was not delivered');
 ...
 assert((await fallbackCheck).version==='1.69.2','fallback channel was not delivered');
 ...
 const verified=await test.verifyRelease({version:'1.70.0',minimumBundled:'1.69.2',modules:[{id:'v170',sha256:digest,source}],assets:{},notes:['test']});
 assert(verified.modules[0].source===source,'verified source was not retained');
 rejected=false;try{await test.verifyRelease({version:'1.70.0',modules:[{id:'bad',sha256:'0'.repeat(64),source}]})}catch(_){rejected=true}
 ...
 const active={version:'1.70.0',minimumBundled:'1.69.2',modules:[{id:'v170',sha256:digest,source}],assets:{},notes:[]};
 ...
 console.log('v1.69.2 updater: JSONP transport, fallback, checksum, staging, and rollback passed');
