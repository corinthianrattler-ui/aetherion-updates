...
assert.equal(feed.release.version, '1.72.10');
...
assert.deepEqual(Array.from(feed.release.modules, module => module.id), ['v177-scroll-repair', 'v178-alexus-equipment-repair', 'v1710-alexus-original-body-map']);
for (const [module, path] of [
  ...
  [feed.release.modules[2], 'patches/v1.72.10-alexus-original-body-map.js'],
]) {
  const expected = fs.readFileSync(path, 'utf8');
  assert.equal(module.source, expected);
  assert.equal(module.sha256, crypto.createHash('sha256').update(expected).digest('hex'));
}
assert(!feed.release.modules.some(module => module.id.includes('v179')), 'the fabricated torso-clone patch must not load');
assert(feed.release.notes.some(note => note.includes('original authored body map')));
assert(feed.release.notes.some(note => note.includes('GOWN_tripo_part_10')));
assert(feed.release.notes.some(note => note.includes('fabricated v1.72.9 torso clone')));
assert.deepEqual(Object.keys(feed.release.assets), [], 'the original-body correction must reuse the packaged GLB unchanged');
...
(async () => {
  ...
  assert.equal(staged.version, '1.72.10');
  assert.equal(updaterWindow.AetherionUpdater.state().staged, '1.72.10');
  console.log('v1.72.10 channel: scroll, equipment persistence, and the original Alexus body map verify and stage through the installed updater');
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
});
