...
const libitaHost={children:[],replaceChildren(...nodes){this.children=nodes},appendChild(node){this.children.push(node)},textContent:''};
const libitaReset={addEventListener(type,handler){this.handler=handler}};
const card={dataset:{},className:'',classList:classes(),innerHTML:'',querySelector(selector){if(selector==='.v171-libita-host')return libitaHost;if(selector==='[data-v171-libita-front]')return libitaReset;return null}};
const header={after(node){modal.card=node}};
const modal={textContent:'Libita Savitas',innerHTML:'<h2>Libita Savitas</h2>',card:null,querySelector(selector){if(selector==='[data-v171-libita-model]')return this.card;if(selector==='.modalHeader')return header;return null},prepend(node){this.card=node}};
const document={
 hidden:false,
 head:{appendChild(){}},
...
const libitaNames=[...Array.from({length:21},(_,index)=>`BODY_${index+1}`),...Array.from({length:7},(_,index)=>`GOWN_${index+1}`)];
let disposed=0,createCalls=[],openCalls=0,equipmentCalls=0,closeCalls=0;
function viewerFor(options,names){
 ...
 return{
  ...
  setYaw(value){this.yaw=value},attach(host){this.host=host},resize(){},fitModel(name){this.fitted=name},render(){},
  dispose(){this.disposed=true;disposed++}
 };
}
const S={
 player:{equipment:{}},v98:{playerMode:'foundation',creator:{height:1,frame:1}},
 people:[{id:'v43_libita_savitas',name:'Libita Savitas',portrait:'libita.webp'}]
};
const V80_RUNTIME={viewer:{dispose(){disposed++}},canvas:{},initializing:{},mountCount:0};
const V98_PLAYER={viewer:null,kind:null,pending:false};
const context={
 console,window:null,document,S,V80_RUNTIME,V98_PLAYER,currentTab:'equipment',Math,
 render(){},setTimeout(){return 1},requestAnimationFrame(){return 1},
 v98PlayerHost:()=>valkorionHost,v98PlayerKind(){},v98ApplyCreator(){},v98Sync(){},v80Sync(){},v97Sync(){},v80ScheduleSync(){},
 ...
 AetherionThree:{async createViewer(options){
  ...
  if(options.glb.endsWith('libita-gothic-gown.glb'))return viewerFor(options,libitaNames);
  throw Error(`unexpected model ${options.glb}`);
 }}
};
context.window=context;
vm.createContext(context);
vm.runInContext(source,context,{filename:'v1.71.0-character-models.js'});

(async()=>{
 const api=context.AetherionCharacterModelsV171;
 assert.equal(api.version,'1.71.0');
 assert.equal(api.mode(),'base');
 assert.equal(context.v98PlayerKind(),'foundation');
 assert.equal(S.people[0].model3d,'assets/v171/libita-gothic-gown.glb');
 ...
 assert(createCalls[0].glb.endsWith('/assets/v171/valkorion-base-lord.glb'));
 ...
 assert.equal(api.mode(),'lord');assert.equal(context.v98PlayerKind(),'royal_male');
 ...
 assert.equal(api.mode(),'armored');assert.equal(context.v98PlayerKind(),'knight');
 viewer=await api.sync();
 assert.equal(createCalls.length,2);
 assert(createCalls[1].glb.endsWith('/assets/v171/valkorion-armored.glb'));
 ...
 const libita=await api.mountLibita();
 assert(libita,'Libita viewer did not mount');
 assert.equal(createCalls.length,3);
 assert(createCalls[2].glb.endsWith('/assets/v171/libita-gothic-gown.glb'));
 assert.equal(libita.yaw,Math.PI/2,'Libita must start front-facing');
 assert.equal(api.state().libita.parts,28);
 assert.equal(api.state().valkorion.ready,false,'Libita must release the large wardrobe viewer on mobile');
 assert.equal(modal.card,card);
 assert(card.classList.values.has('v171-libita-ready'));

 assert.equal(context.openPerson('v43_libita_savitas'),'person-opened');
 assert.equal(context.v16PersonEquipment('v43_libita_savitas'),'equipment-opened');
 ...
 assert.equal(api.state().libita.ready,false,'closing Libita must release her large viewer');
 console.log('v1.71.0 character models: base/Lord/armor switching, fitted-part contracts, Libita profile mount, and viewer disposal passed');
