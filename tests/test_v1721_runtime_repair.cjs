...
const source=fs.readFileSync('patches/v1.72.1-runtime-repair.js','utf8');
...
vm.runInContext(source,context,{filename:'v1.72.1-runtime-repair.js'});

assert.equal(context.AetherionRuntimeRepairV172.version,'1.72.1');
...
console.log('v1.72.1 runtime repair: Continue, menu paths, tap color, and scheduled mounting passed');
