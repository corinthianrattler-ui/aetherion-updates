...
const updater=fs.readFileSync(path.join(root,'patches/v1.72.1-safe-updater.js'),'utf8');
const center=fs.readFileSync(path.join(root,'patches/v1.72.1-update-center.js'),'utf8');

assert.match(updater,/const VERSION='1\.72\.1'/);
assert.match(updater,/const BUNDLED_VERSION='1\.72\.1'/);
assert.match(updater,/const ANDROID_BUILD=190/);
...
console.log('v1.72.1 menu update center and no-observer performance checks passed');
