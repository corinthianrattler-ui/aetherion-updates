...
const source=fs.readFileSync('patches/v1.72.2-character-models.js','utf8');
function classes(){
...
vm.runInContext(source,context,{filename:'v1.72.2-character-models.js'});

(async()=>{
 const api=context.AetherionCharacterModelsV172;
 assert.equal(api.version,'1.72.2');
 ...
 assert(stage.classList.values.has('v172-model-error'),'the stage must expose the real model error');
 ...
 assert(source.includes('.v80-stage.v172-model-error .v80-valkorion-host'),'model failures must be visible instead of hiding behind the portrait');
 console.log('v1.72.2 character models: Android binary reads, local-only paths, render hook, fitted model switching, and visible errors passed');
