...
const updater=fs.readFileSync(path.join(root,'patches/v1.72.2-safe-updater.js'),'utf8');
const center=fs.readFileSync(path.join(root,'patches/v1.72.2-update-center.js'),'utf8');

assert.match(updater,/const VERSION='1\.72\.2'/);
assert.match(updater,/const BUNDLED_VERSION='1\.72\.2'/);
assert.match(updater,/const ANDROID_BUILD=191/);
...
assert.match(center,/DOWNLOAD FULL APK/);
assert.match(center,/Aetherion_Reforged_v1\.72\.2_ANDROID_ASSET_FIX\.apk/);
...
console.log('v1.72.2 menu update center and no-observer performance checks passed');
