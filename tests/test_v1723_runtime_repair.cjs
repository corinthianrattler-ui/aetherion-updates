...
const source=fs.readFileSync('patches/v1.72.3-runtime-repair.js','utf8');
...
let clearedTimer=null,pausedVideo=0,unloadedVideo=0;
let installedStyle=null;
const launcher={remove(){removeCalls++}};
const introVideo={pause(){pausedVideo++},removeAttribute(name){if(name==='src')unloadedVideo++},load(){unloadedVideo++}};
const document={
 readyState:'complete',documentElement:{},head:{appendChild(node){installedStyle=node}},
 getElementById(id){if(id==='app')return{};if(id==='aetherion-update-launcher')return launcher;if(id==='v12IntroVideo')return introVideo;return null},
...
vm.runInContext("let v12IntroActive=true,v12IntroTimer=77,v12IntroReturnTab='equipment'",context);
vm.runInContext(source,context,{filename:'v1.72.3-runtime-repair.js'});

assert.equal(context.AetherionRuntimeRepairV172.version,'1.72.3');
assert.equal(startCalls,1,'an existing timeline must open at the title menu');
assert.equal(vm.runInContext('v12IntroActive',context),false,'opening-film guard must be cleared before the title menu is shown');
assert.equal(vm.runInContext('v12IntroTimer',context),null,'opening-film timer must be retired');
assert.equal(clearedTimer,77);
assert.equal(pausedVideo,1);
assert.equal(unloadedVideo,2);
...
assert.equal(vm.runInContext('v12IntroActive',context),false,'Continue must leave the opening-film guard inactive');
...
console.log('v1.72.3 runtime repair: Continue, menu paths, tap color, and scheduled mounting passed');
