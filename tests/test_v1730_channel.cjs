...
assert.equal(feed.release.version,'1.73.7');
...
const expected=[
 ['v1737-stable-bundle','patches/v1.73.7-stable-bundle.js'],
];
...
assert(feed.release.notes.some(note=>note.includes('clear top-right HUD pocket')));
assert(feed.release.notes.some(note=>note.includes('moving sun')));
...
(async()=>{
 ...
 assert.equal(verified.version,'1.73.7');
 assert.equal(verified.build,195);
 const staged=updaterWindow.AetherionUpdater.stage(verified);
 assert.equal(staged.version,'1.73.7');
 assert.equal(updaterWindow.AetherionUpdater.state().staged,'1.73.7');
 console.log('v1.73.7 channel: corrected sky-dial placement and the complete prior update verify and stage through the installed build-195 safe updater');
