...
assert.equal(feed.release.version,'1.73.0');
...
assert(feed.release.notes.some(note=>note.includes('redraws, lists, schedules, and daily ticks')));
...
(async()=>{
 ...
 assert.equal(verified.version,'1.73.0');
 assert.equal(verified.build,195);
 const staged=updaterWindow.AetherionUpdater.stage(verified);
 assert.equal(staged.version,'1.73.0');
 assert.equal(updaterWindow.AetherionUpdater.state().staged,'1.73.0');
 console.log('v1.73.0 channel: five exact modules verify and stage through the installed build-195 safe updater');
