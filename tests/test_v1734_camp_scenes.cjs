...
function openModal(html){modal=html;elements.clear();elements.set('v1734CampVideo',media())}
...
const sandbox={console,document,openModal,closeModal(){},persist(){persists++},v24OpenCamp(){campOpens++},v19Block(title,text){blocks.push({title,text})},S:{world:{day:2,hour:4},v24:{camp:{active:false,placements:[],watchers:[],cookReady:false}}},
 ...
 v24AssignWatch(){guardCalls++;this.S.v24.camp.watchers=['guard_a','guard_b']},v24RemovePlot(i){this.S.v24.camp.placements[i]=null}};
sandbox.v14Now=()=>sandbox.S.world.day*24+sandbox.S.world.hour;sandbox.window=sandbox;sandbox.globalThis=sandbox;
vm.runInNewContext(fs.readFileSync('patches/v1.73.4-camp-scenes.js','utf8'),sandbox,{filename:'v1.73.4-camp-scenes.js'});
const api=sandbox.AetherionV1734CampScenes;
assert(api);assert.deepEqual(Object.fromEntries(Object.entries(api.scenes)),{make:'make-camp.mp4',strike:'strike-camp.mp4',food:'camp-food.mp4',sleep:'camp-sleep.mp4'});assert.match(sandbox.v24CampMap(),/MAKE CAMP/);
...
assert.match(modal,/make-camp\.mp4/);assert.match(modal,/poster="[^"]+make-camp\.webp"/);assert.match(modal,/>SKIP<\/button>/);assert.doesNotMatch(modal,/\scontrols(?:\s|=|>)/);assert.doesNotMatch(modal,/>PLAY<|RETURN TO CAMP/);assert.equal(elements.get('v1734CampVideo').controls,false);
assert.equal(elements.get('v1734CampVideo').played,2,'a blocked sound autoplay must retry muted');assert.equal(elements.get('v1734CampVideo').muted,true);
...
sandbox.v24AssignWatch();assert.equal(guardCalls,0);assert.equal(blocks.at(-1).title,'NO ESTABLISHED CAMP');
sandbox.S.v24.camp.active=true;sandbox.v24AssignWatch();assert.equal(guardCalls,0);assert.equal(blocks.at(-1).title,'NO WATCH POST');
sandbox.S.v24.camp.placements[1]={kind:'watch'};sandbox.v24AssignWatch();assert.equal(guardCalls,1);assert.deepEqual(sandbox.S.v24.camp.watchers,['guard_a','guard_b']);
...
console.log('v1.73.4 camp scenes: autoplay with muted Android fallback, first-frame posters, hidden native overlay, skip-only UI, Watch Post guard gate, realistic time, and exact routing passed');
