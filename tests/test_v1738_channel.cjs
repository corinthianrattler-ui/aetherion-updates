...
assert.equal(feed.release.version,'1.73.8');
...
const expected=[
 ['v1738-stable-bundle','patches/v1.73.8-stable-bundle.js'],
];
...
assert(feed.release.notes.some(note=>note.includes('full-body portraits')));
...
(async()=>{
 ...
 assert.equal(verified.version,'1.73.8');
 assert.equal(verified.build,195);
 const staged=updaterWindow.AetherionUpdater.stage(verified);
 assert.equal(staged.version,'1.73.8');
 assert.equal(updaterWindow.AetherionUpdater.state().staged,'1.73.8');
 console.log('v1.73.8 channel: portrait resilience and the complete prior update verify and stage through the installed build-195 safe updater');
