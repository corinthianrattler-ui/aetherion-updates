...
vm.runInContext(fs.readFileSync('patches/v1.73.8-stable-bundle.js','utf8'),context,{filename:'v1.73.8-stable-bundle.js'});
...
assert.equal(resilienceApi?.version,'1.73.8');
...
assert.equal(get('S.meta.v1738PortraitResilience.version'),'1.73.8');
...
assert(knightCards.includes('custom%2Fnpc-portraits%2Fv173'));
...
const alreadyMarkedClone=get(`(()=>{let save=structuredClone(S);delete save.meta.v1738PortraitResilience;save.meta.v173LivingPortraits={version:'1.73.0'};save.meta.v1731KnightDiversity={version:'1.73.1'};for(const p of save.people)if(p.role==='Lesser Knight'||p.role==='Company Captain')p.portrait='assets/dynasty/male_adult_1.webp';save=migrateState(save);return{marker:save.meta.v1738PortraitResilience,audit:AetherionV1731KnightDiversity.auditState(save),cards:peopleCards(save.people)}})()`);
assert.equal(alreadyMarkedClone.marker.version,'1.73.8');
...
assert(alreadyMarkedClone.cards.includes('custom%2Fnpc-portraits%2Fv173'));
...
console.log(`v1.73.8 full-game runtime: ${scripts.length} packaged scripts, marked-clone save repair, encoded full-body delivery, corrected sky-dial placement, seasonal sleep-to-dawn, moon phases, waiting, wakefulness, guard film, Watch Post gate, diversity, residents, recruits, production, views, and migration passed`);
