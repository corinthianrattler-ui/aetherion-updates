...
assert.equal(feed.release.version,'1.74.0');
assert.equal(feed.release.build,196);
...
assert.equal(feed.release.apkSha256,'08791fdc068bcfa6a1809961dc01954c3f1d614cf5a684f4b33fd2308be0d3c7');
assert.equal(feed.release.apkSize,543684738);
assert(feed.release.apkUrl.endsWith('/v1.74.0/Aetherion_Reforged_v1.74.0_CLEAN_MAINTENANCE_FULL.apk'));
assert(feed.release.notes.some(note=>note.includes('preserves voices')));
...
(async()=>{
 ...
 assert.equal(verified.version,'1.74.0');
 assert.equal(verified.build,196);
 assert.equal(verified.requiresApk,true);
 console.log('legacy build-195 updater accepts the full-APK v1.74.0 handoff without staging another patch');
