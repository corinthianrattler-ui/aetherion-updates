...
vm.runInNewContext(fs.readFileSync('patches/v1.74.0-safe-updater.js','utf8'),sandbox,{filename:'v1.74.0-safe-updater.js'});

const api=sandbox.AetherionUpdater;
assert(api,'updater API missing');
assert.equal(api.safeUpdaterVersion,'1.74.0');
assert.equal(api.bundledVersion,'1.74.0');
assert.equal(api.androidBuild,196);
assert.equal(api._test.appId,'aetherion-reforged');
assert.equal(api._test.database,'aetherion-reforged-updates');
assert.equal(api._test.compareVersions('1.74.1','1.74.0'),1);
...
const release={version:'1.74.1'};
const valid={schema:2,appId:'aetherion-reforged',version:'1.74.1',files:[{
...
assert.throws(()=>api._test.validateManifest({schema:2,appId:'aetherion-reforged',version:'1.74.1',files:[],deletePaths:['game.js']},release),/Unsafe/);
assert.throws(()=>api._test.validateManifest({schema:2,appId:'aetherion-reforged',version:'1.74.1',files:[],deletePaths:['..\/save.json']},release),/Unsafe/);

console.log('v1.74.0 updater: signed-channel identity, safe add/replace/delete boundaries, exact legacy-cache cleanup, and save preservation passed');
