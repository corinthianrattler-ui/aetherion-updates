...
assert(api.safeUpdaterVersion==='1.72.5','updater API missing');
assert(api.bundledVersion==='1.72.5','bundled version mismatch');
assert(api.androidBuild===194,'Android build mismatch');
assert(test.compareVersions('1.72.6','1.72.5')===1,'version ordering failed');
assert(test.compareVersions('1.72.5','1.72.5')===0,'version equality failed');
...
(async()=>{
 ...
 const release={version:'1.72.5',build:194,minimumBundled:'1.72.5',releasedAt:'2026-09-09T00:00:00Z',notes:['transport test'],modules:[],assets:{}};
 ...
 assert((await primaryCheck).version==='1.72.5','decoded channel was not delivered');
 ...
 assert((await fallbackCheck).version==='1.72.5','fallback channel was not delivered');
 ...
 const verified=await test.verifyRelease({version:'1.72.6',build:194,minimumBundled:'1.72.5',modules:[{id:'v176',sha256:digest,source}],assets:{},notes:['test']});
 assert(verified.modules[0].source===source,'verified source was not retained');
 rejected=false;try{await test.verifyRelease({version:'1.72.6',modules:[{id:'bad',sha256:'0'.repeat(64),source}]})}catch(_){rejected=true}
 assert(rejected,'checksum mismatch was accepted');
 rejected=false;try{test.sanitizeRelease({version:'1.72.6',modules:[],assets:{x:'https://example.com/evil'}})}catch(_){rejected=true}
 assert(rejected,'untrusted asset origin was accepted');
 const active={version:'1.72.6',build:194,minimumBundled:'1.72.5',modules:[{id:'v176',sha256:digest,source}],assets:{},notes:[]};
 test.writeState({schema:1,active,previous:null,staged:null});
 localStorage.setItem('aetherion_safe_update_boot_v1',JSON.stringify({status:'loading',version:'1.72.6'}));
 ...
 console.log('v1.72.5 updater: JSONP transport, fallback, checksum, staging, and rollback passed');
