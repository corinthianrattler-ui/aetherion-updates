...
assert.equal(feed.release.version, '1.72.9');
...
assert.deepEqual(Array.from(feed.release.modules, module => module.id), ['v177-scroll-repair', 'v178-alexus-equipment-repair', 'v179-alexus-torso-foundation']);
for (const [module, path] of [
  ...
  [feed.release.modules[2], 'patches/v1.72.9-alexus-torso-foundation.js'],
]) {
...
assert(feed.release.notes.some(note => note.includes('torso foundation')));
assert(feed.release.notes.some(note => note.includes('v1.72.8')));
assert(feed.release.notes.some(note => note.includes('v1.72.7')));
assert.deepEqual(Object.keys(feed.release.assets), [], 'the torso correction must not require another model or APK download');
...
(async () => {
  ...
  assert.equal(staged.version, '1.72.9');
  console.log('v1.72.9 channel: scroll, equipment persistence, and torso foundation verify and stage together through the installed updater');
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
});
