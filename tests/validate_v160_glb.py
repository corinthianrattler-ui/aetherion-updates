...
def main(path: Path) -> None:
    ...
    check(len(nodes) == len(node_names) == 32, "node names must be unique")
    check(len(document["meshes"]) == 32, "each switchable node must own one mesh")
    ...
    check(set(document["scenes"][0]["nodes"]) == set(range(32)), "scene must include every modular node")
    ...
    for index, node in enumerate(nodes):
        ...
        check(-0.03 <= low[1] <= high[1] <= 1.06, f"{name}: vertical fit escaped the body envelope")
        check(-0.21 <= low[2] <= high[2] <= 0.21, f"{name}: depth fit escaped the body envelope")
    ...
    print(
        f"{path.name}: 32 fitted nodes, {len(EXPECTED_GEAR)} equipment pieces, "
        f"{len(data) / 1048576:.2f} MiB — structural checks passed"
    )


if __name__ == "__main__":
    model = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("assets/v106/valkorion_modular.glb")
    main(model)
