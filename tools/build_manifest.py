...
def main() -> None:
    ...
    camp_v1732 = [
        str(path.relative_to(ROOT))
        for path in sorted(CAMP_V1732_ROOT.glob("*.mp4"))
    ]
    assert len(camp_v1732) == 4
    paths = [
    ...
    print(f"manifest.json: wrote {len(paths)} verified payload entries for v1.73.3")
